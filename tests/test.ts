...
const api = new Bandcamp({
	id: String(process.env.BANDCAMP_CLIENT_ID),
	secret: String(process.env.BANDCAMP_CLIENT_SECRET),
});

const date = new Date();
const start_time = new Date('2018-01-01');
const package_ids = String(process.env.BANDCAMP_PACKAGE_IDS).split('|');
...
test('BANDCAMP_CLIENT_ID', () => {
	...
	if (!isCI) {
		console.log(process.env.BANDCAMP_CLIENT_ID);
	}
...
test('BANDCAMP_CLIENT_SECRET', () => {
	...
	if (!isCI) {
		console.log(process.env.BANDCAMP_CLIENT_SECRET);
	}

	assert.is.not(actual?.length, 0);
});
...
test('BANDCAMP_BAND_ID', () => {
	...
	if (!isCI) {
		console.log(process.env.BANDCAMP_BAND_ID);
	}
...
test('BANDCAMP_SALE_ID', () => {
	...
	if (!isCI) {
		console.log(process.env.BANDCAMP_SALE_ID);
	}
...
test('BANDCAMP_PACKAGE_IDS', () => {
	...
	if (!isCI) {
		console.log(process.env.BANDCAMP_PACKAGE_IDS);
	}
